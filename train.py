...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=101, help='Number of epochs to train on')
    ...
    train_loader = DataLoader(train_data, batch_size=opts.batch_size, shuffle=True, collate_fn=train_data.collate)
    ...
    for epoch in range(opts.epochs):
        epoch_class_loss = []
        epoch_loc_loss = []
        for i, (images, gt_annots) in enumerate(train_loader):
            optimizer.zero_grad()
            ...
            losses.append(float(local_loss+class_loss))
        ...
        print(
            'epoch: {} | cls loss: {:1.15f} | loc loss: {:1.15f} | loss: {:1.5f}'.format(
                epoch,  np.mean(epoch_class_loss), np.mean(epoch_loc_loss), np.mean(losses)))
